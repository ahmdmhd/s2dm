...
from graphql import ExecutionResult, GraphQLError, TypeKind, build_schema, get_introspection_query, graphql_sync
...
def get_all_named_types(schema: GraphQLSchema) -> list[GraphQLNamedType]:
    ...
    """
    return [type_ for type_ in schema.type_map.values() if isinstance(type_, GraphQLNamedType) and not type_.name.startswith('__')]
...
def get_directive_arguments(field: GraphQLField, directive_name: str) -> dict[str, Any]:
    ...
    logging.debug(f"Directive '{directive_name}' not found in field '{field.name}'.")
    return {}
