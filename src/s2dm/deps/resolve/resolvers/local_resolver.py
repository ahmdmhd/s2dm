...
class LocalResolver(Resolver):
    ...
    def resolve(self, dependency: DependencyEntry) -> ResolvedDependencySource:
        ...
        if not source_directory.exists():
            raise DependencySourceError(f"Dependency source directory does not exist: {source_directory}")
        if not source_directory.is_dir():
            raise DependencySourceError(f"Dependency source must be a directory: {source_directory}")

        artifact_path = source_directory / dependency.artifact
        if not artifact_path.exists():
            raise DependencySourceError(f"Dependency artifact not found: {artifact_path}")
        if not artifact_path.is_file():
            raise DependencySourceError(f"Dependency artifact must be a file: {artifact_path}")
